...
class SoftwareProductSubDetails extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('subDetailsModel/SoftwareProductSubDetailsModel', 'SPModel');
    }
    ...
    public function softwareProductPage($id = NULL)
    {
        // echo $id;
        // die;
        $data['technology_id'] = $id;
        $data['title'] = 'Software Products Details';
        $data['mast_load'] = 'ALOAD';
        // echo $id;
        // die;
        $this->load->master_template('subDetails/Technology/technologySubDetails', $data);
        // $this->load->view('subDetails/SoftwareProduct/softwareProductSubDetails', $data);
    }
    ...
    public function create()
    {
        ...
        if (!empty($_FILES['image']['name'])) {
            ...
            if (!in_array(strtolower($extension), $allowed)) {
                ...
                if ($_FILES["image"]["error"] > 0) {
                    ...
                    if (!move_uploaded_file($_FILES["image"]["tmp_name"], './uploads/softwareProductSubDetails/' . $fileName)) {
                        $data['inputerror'][] = 'image';
        ...
        $insertData = $this->SPModel->createData($dataToInsert);
    ...
    public function fetchDatafromDatabase()
    {
        ...
        $resultList = $this->SPModel->fetchAllData(
            '*',
            'softwareproductsubdetails',
        ...
        foreach ($resultList as $key => $value) {
            ...
            $result['data'][] = array(
                ...
                '<img src="' . base_url('uploads/softwareProductSubDetails/' . htmlspecialchars($value['image'])) . '" alt="Image" style="width: 100px;">',
                $button
            );
        }
    ...
    public function getEditData()
    {
        ...
        $resultData = $this->SPModel->fetchSingleData('*', 'softwareproductsubdetails', array('id' => $id));
    ...
    public function update()
    {
        ...
        if (!empty($_FILES['image']['name'])) {
            ...
            $uploadPath = './uploads/softwareProductSubDetails/' . $fileName;
            ...
            if ($old_image && file_exists('./uploads/softwareProductSubDetails/' . $old_image)) {
                unlink('./uploads/softwareProductSubDetails/' . $old_image);
            }
        ...
        $update = $this->SPModel->updateData('softwareproductsubdetails', $data, array('id' => $id));
    ...
    {
        $id = $this->input->post('id');
        $dataDelete = $this->SPModel->deleteData('softwareproductsubdetails', array('id' => $id));
