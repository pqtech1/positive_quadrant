    ...
    public function index()
    {
        ...
        public function edit_about($id)
        {
             ...
             public function updateDirect($id)
            {
               ...
               $fileName  =  $_POST['old_img'];
                ...
                {   
                        ...
                        $allowed = array('jpg', 'jpeg','png','gif');
                        $extension = pathinfo($_FILES['about_desc']['name'], PATHINFO_EXTENSION);
                        if(!in_array(strtolower($extension), $allowed))
                        {
                               $data['inputerror'][] = 'image';
                               $data['error_string'][] = 'Image extension should be jpeg,gif,png,pdf';
                        ...
                        {
                            $maxsize = 2097152;
                            if($_FILES['about_desc']['size'] >= $maxsize)
                            {
                                $data['inputerror'][] = 'image';
                                $data['error_string'][] = 'File too large. File must be less than 2 megabytes';
                                $data['status'] = FALSE;
                            ...
                            if ($_FILES["about_desc"]["error"] > 0) {
                            //$data =  json_encode(array('type'=>'error', 'msg'=>"Return Code: " . $_FILES["file"]["error"][$i])); 
                            $data['inputerror'][] = 'image';
                            $data['error_string'][] = ''.$_FILES["about_desc"]["error"].'';
                            $data['status'] = FALSE;
                            echo json_encode($data);
                            exit();
                            } else {
                                   
                                    $name = explode(".", $_FILES['about_desc']['name']);
                                    $fileName = $name[0].time().'.'.$name[1];
                                   
                                    move_uploaded_file($_FILES["about_desc"]["tmp_name"],
                                "uploads/about/" . $fileName);
                ...
                else
                {
                    ...
                    // $_POST;
                     ...
                     $data = array('about_desc' => $this->input->post('about_desc'),
                                    'status'=> $this->input->post('status')
                            );
                        $this->about->update_about($id, $data);
           ...
           public function delete_about($id)
          {
                    ...
                    $data  = $this->about->delete_about($id);
