...
class IndustriesSubDetails extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('subDetailsModel/IndustriesSubDetailsModel', 'IndusModel');
    }
    ...
    public function industryPage($id = NULL)
    {
        // echo $id;
        // die;
        $data['technology_id'] = $id;
        $data['title'] = 'Industry Sub Details';
        $data['mast_load'] = 'ALOAD';
        // echo $id;
        // die;
        $this->load->master_template('subDetails/Technology/technologySubDetails', $data);
        // $this->load->view('subDetails/Industry/industrySubDetails', $data);
    }
    ...
    public function create()
    {
        ...
        if (!empty($_FILES['image']['name'])) {
            ...
            if (!in_array(strtolower($extension), $allowed)) {
                ...
                if ($_FILES["image"]["error"] > 0) {
                    ...
                    if (!move_uploaded_file($_FILES["image"]["tmp_name"], './uploads/industriessubdetails/' . $fileName)) {
                        $data['inputerror'][] = 'image';
        ...
        $insertData = $this->IndusModel->createData($dataToInsert);
    ...
    public function fetchDatafromDatabase()
    {
        ...
        $resultList = $this->IndusModel->fetchAllData(
            '*',
            'industriessubdetails',
        ...
        foreach ($resultList as $key => $value) {
            ...
            $result['data'][] = array(
                ...
                '<img src="' . base_url('uploads/industriessubdetails/' . htmlspecialchars($value['image'])) . '" alt="Image" style="width: 100px;">',
                $button
            );
        }
    ...
    public function getEditData()
    {
        ...
        $resultData = $this->IndusModel->fetchSingleData('*', 'industriessubdetails', array('id' => $id));
    ...
    public function update()
    {
        ...
        if (!empty($_FILES['image']['name'])) {
            ...
            $uploadPath = './uploads/industriessubdetails/' . $fileName;
            ...
            if ($old_image && file_exists('./uploads/industriessubdetails/' . $old_image)) {
                unlink('./uploads/industriessubdetails/' . $old_image);
            }
        ...
        $update = $this->IndusModel->updateData('industriessubdetails', $data, array('id' => $id));
    ...
    {
        $id = $this->input->post('id');
        $dataDelete = $this->IndusModel->deleteData('industriessubdetails', array('id' => $id));
